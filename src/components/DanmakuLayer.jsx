...
const DanmakuItem = React.memo(({ dm, settings, onAnimationEnd, aaMode, aaOverrideMap, setHoveredImage, onImageClick, onTruncationClick }) => {
    // Determine AA status
    ...
    return (
        ...
        >
            {/* Text Row */}
            <div className={`flex items-center gap-1 ${isAA ? "font-aa text-[0.8em] leading-none" : ""}`} style={{ whiteSpace: isAA ? 'pre' : undefined }}>
                {dm.nodes ? dm.nodes.filter(n => n.type !== 'image').map((node, idx) => {
                    if (node.type === 'placeholder') {
                        const hasContent = node.content && node.content.length > 0;
                        return (
                            <span
                                key={idx}
                                className={`shrink-0 ${hasContent ? 'cursor-pointer pointer-events-auto text-cyan-400 hover:text-cyan-300 hover:underline' : 'text-cyan-400'}`}
                                onMouseEnter={hasContent ? (e) => setHoveredImage({ url: node.content, x: e.clientX, y: e.clientY }) : undefined}
                    ...
                    if (node.type === 'image_error') {
                        return <span key={idx} className="shrink-0 text-gray-400" style={{ fontSize: `${settings.fontSize * 0.7}px` }}>{node.text}</span>;
                    }
                    return <span key={idx} className="shrink-0">{node.text}</span>;
                }) : (
            ...
            {dm.nodes && dm.nodes.some(n => n.type === 'image') && (
                ...
                >
                    {dm.nodes.filter(n => n.type === 'image').map((node, idx) => (
                        <div 
                            key={idx} 
                            className={`shrink-0 relative ${onImageClick ? 'cursor-pointer pointer-events-auto hover:opacity-80' : 'pointer-events-none'}`}
                            style={{ height: node.height || `${settings.fontSize * 4}px` }}
                        ...
                        >
                            <img 
                                ...
                                onError={(e) => {
                                    ...
                                    container.innerHTML = `<span style="color: #ff6b6b; font-size: ${settings.fontSize * 0.7}px; white-space: nowrap;">[画像エラー]</span>`;
                                }}
                            />
                        </div>
                    ))}
