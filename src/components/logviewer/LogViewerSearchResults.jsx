...
const LogViewerSearchResults = ({
    ...
    const getTitle = () => {
        switch (activeFilter) {
            case 'image': return '画像付きコメント';
            case 'popular': return '人気のコメント（返信3件以上）';
            case 'url': return 'URLを含むコメント';
            case 'video': return '動画URLを含むコメント';
            case 'aa': return 'アスキーアート';
            default: return `「${activeSearchQuery}」の検索結果`;
        }
    };
            ...
            >
                ...
                <div className="bg-gray-800 px-4 py-3 border-b border-gray-700 flex justify-between items-center shrink-0">
                    <div className="flex items-center gap-3">
                        {activeFilter !== 'none' ? <Filter size={18} className="text-blue-400" /> : <Search size={18} className="text-blue-400" />}
                ...
                <div className="flex-1 overflow-y-auto bg-gray-900">
                    {displayResults.length === 0 ? (
                        <div className="p-8 text-center text-gray-500">
                            {activeFilter !== 'none' ? <Filter size={32} className="mx-auto mb-3 opacity-50" /> : <Search size={32} className="mx-auto mb-3 opacity-50" />}
                            <p>結果が見つかりませんでした</p>
                        </div>
                    ) : (
