...
const LogCommentItemWrapper = React.memo(({ node, isActive, isHighlighted, currentLogicalTime, onAnchorClick, onUrlLoad, showImages, setZoomedImage, timeOffset, formatTime, onRowClick, totalComments, onIdClick, settings, onReplyCountClick, aaOverride, className = "" }) => {
    return (
        <LogCommentItem
            comment={node}
            isActive={isActive}
...
const LogViewer = ({
    ...
    const handleFileSelect = React.useCallback((newFileId) => {
    ...
    const {
        popupStack,
        logContextMenu, setLogContextMenu,
        handleClosePopup,
        clearPopups,
        handlePopupRowClick,
        handleAnchorClick,
    ...
    const {
        ...
        searchHistoryIndex, setSearchHistoryIndex,
        searchHistory,
        addToHistory,
        handleSearchKeyDown,
        displayResults
        ...
        <div className="flex-1 min-h-0 bg-gray-900 overflow-hidden relative flex flex-row">
            ...
            <div ref={containerRef} className="flex-1 flex flex-col min-w-0 min-h-0 bg-gray-900 border-x border-gray-800 shadow-xl relative">
                ...
                <CommentList
                    ...
                    onAnchorClick={(e, resNum, sourceFileId) => handleAnchorClick(e, resNum, sourceFileId, false)}
                    onIdClick={handleIdClick}
                    ...
                    onScrollIndexChange={(index) => { currentScrollIndexRef.current = index; }}
                    extraRowProps={{ 
                        settings: logSettings,
            ...
            {/* Reuse UserHistoryModal if needed */}
             {userHistoryId && (
                 <div className="fixed inset-0 z-70 bg-black/60 flex items-center justify-center p-4 animate-fade-in" onClick={() => setUserHistoryId(null)}>
                     <div className="w-full max-w-3xl h-[85vh] bg-gray-900 rounded-lg shadow-2xl overflow-hidden border border-gray-700" onClick={e => e.stopPropagation()}>
                        <UserHistoryModal
                            isSidebarMode={true}
