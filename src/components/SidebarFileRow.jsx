...
const SidebarFileRow = ({ file, index, handleToggleFileVisibility, handleRemoveFile }) => {
    ...
    const dateStr = file.startDate ? new Date(file.startDate).toLocaleString('ja-JP', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) : '';
            ...
            <div className="flex items-start gap-2 overflow-hidden pointer-events-none">
                ...
                <div className="flex flex-col overflow-hidden pointer-events-auto">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`font-mono break-all ${file.isVisible === false ? 'text-gray-600' : 'text-gray-300'}`} title={file.name}>{file.name}</span>
                    </div>
                    {dateStr && <span className={`text-gray-500 text-[10px] ml-6 ${file.isVisible === false ? 'opacity-30' : ''}`}>{dateStr} ({file.rawComments.length})</span>}
                </div>
            </div>
            <button onClick={() => handleRemoveFile(file.id)} className="text-gray-500 hover:text-red-400 p-1 transition shrink-0 pointer-events-auto" title="削除"><Trash2 size={14} /></button>
        </div>
    );
};
