...
const CommentItem = ({
    ...
    const isFuture = !isLogMode && currentLogicalTime !== undefined && effectiveTime > currentLogicalTime;
    ...
    return (
        <div
            ...
            className={`comment-item-row cursor-pointer transition-colors duration-200 ${shouldHighlightActive ? 'bg-blue-600/20' : isHighlighted ? 'bg-yellow-500/20' : isFuture ? 'opacity-40 grayscale-50' : 'hover:bg-gray-800/60'} ${isLogMode ? "text-base py-3 px-3" : "text-sm pb-2 pt-2 px-2"} ${className}`}
            style={{
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[10px] text-gray-400 flex justify-between gap-2 mb-0.5 items-baseline">
                        <span className="flex items-center gap-1 min-w-0 flex-1">
                            <span className="font-bold shrink-0">{comment.originalResNum || comment.resNum}:</span>
                    ...
                    {/* Body */}
                    <div className={`text-white break-all whitespace-pre-wrap text-sm leading-snug ${isAA ? 'font-aa text-[12px]' : ''}`}>
                        <CommentContent
                            text={comment.text}
                            comment={comment}
