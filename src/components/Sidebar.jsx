...
import { Settings, ChevronUp, ChevronDown, Upload, Clock, RefreshCw, Plus, Edit2, X, Link as LinkIcon, Pipette, ArrowDown, Hash, Ban, FileText, Trash2, Eye, EyeOff, GripVertical } from 'lucide-react';
...
const Sidebar = ({
    ...
    const customFormatTime = React.useCallback((seconds) => {
        ...
        enrichedList.forEach(c => {
            ...
            if (myResNum && myFileId) {
                ...
                if (replyMap.has(key)) {
                    ...
                    const enrichedReplies = originalReplies.map(origR => enrichedLookup.get(origR.id)).filter(Boolean);
    ...
    const handleAnchorClick = (e, targetResNum, isNested = false) => {
        ...
        // Find target comment
        const targetComment = enrichedComments.find(c => (c.originalResNum || c.resNum) === targetResNum);
        if (targetComment) {
             const baseRect = e?.target?.getBoundingClientRect() || { left: 100, bottom: 100 };
             const rowElement = e?.target?.closest('.comment-item-row');
             const rowRect = rowElement?.getBoundingClientRect();

             setPopupStack(prev => [...prev, {
                 type: 'anchor',
    ...
    const handleReplyCountClick = (e, comment, isNested = false) => {
        ...
        if (comment.replies && comment.replies.length > 0) {
             const baseRect = e.target.closest('.reply-count-indicator')?.getBoundingClientRect() || e.target.getBoundingClientRect();
             const rowElement = e.target.closest('.comment-item-row');
             const rowRect = rowElement?.getBoundingClientRect();
             ...
             if (!hasAnchors) {
                 ...
                 const findRoots = (c, visited) => {
                     ...
                     matches.forEach(m => {
                         ...
                         const parent = enrichedComments.find(ec => (ec.originalResNum || ec.resNum) === targetResNum && ec.sourceFileId === c.sourceFileId);
                         if (parent) {
    ...
    const handleSetCmStart = (time) => {
        const startSec = startTimeStr ? startTimeStr.split(':').reduce((acc, v) => acc * 60 + Number(v), 0) : 0;
    ...
    const scrollToCmSettings = () => {
        if (settingsScrollRef.current && cmSettingsRef.current) {
            cmSettingsRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
        }
    };
    ...
    return (
        <div className="flex flex-col bg-gray-900 border-l border-gray-700 h-full" style={{ width: sidebarWidth }}>
            ...
            {showSettingsPanel && (
                <>
                    <div ref={settingsScrollRef} className="bg-gray-800 border-b border-gray-700 overflow-y-auto max-h-[85vh] scrollbar-thin shrink-0">
                        <div className="p-4 space-y-6">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-center justify-between bg-gray-800 p-2 rounded border border-gray-700">
                                        ...
                                        <div className="flex flex-col items-center gap-1">
                                            ...
                                            <TimeInput value={videoStartTimeStr} onChange={setVideoStartTimeStr} showHours={true} placeholder="00:00" />
                                        </div>
                                    </div>
                            ...
                            <div className="p-4 space-y-5 border-t border-gray-700">
                                <div className="space-y-2">
                                    <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wider">キーボード操作設定</h4>
                                    <div className="space-y-1">
                                        ...
                                        <input type="range" min="1" max="60" step="1" value={skipSeconds || 5} onChange={e => setSkipSeconds(parseInt(e.target.value))} className="w-full h-1 bg-gray-600 rounded accent-blue-500" />
                                    </div>
                                </div>
                                {/* CM Settings */}
                                <div ref={cmSettingsRef} className="space-y-2">
                                    <div className="flex flex-col gap-2 bg-gray-800 p-2 rounded border border-gray-700">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {renderTimeInput(cmStartMode, cmStartInput, setCmStartInput, cmStartMode === 'log' ? "00:00:00" : "00:00")}
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {renderTimeInput(cmEndMode, cmEndInput, setCmEndInput, cmEndMode === 'log' ? "00:00:00" : "00:00")}
                                        </div>
                                        ...
                                        {editingCmIndex !== null ? (
                                            <div className="flex gap-2 mt-1">
                                                <button
                                                    onClick={() => {
                                                        updateCmRange(editingCmIndex, cmStartMode, cmStartInput, cmEndMode, cmEndInput, startTimeStr);
                                            ...
                                            <button
                                                onClick={() => addCmRangeSmart(cmStartMode, cmStartInput, cmEndMode, cmEndInput, startTimeStr)}
                                    ...
                                    {cmRanges.length > 0 && (
                                        <div className="space-y-1 mt-1">
                                            {cmRanges.map((range, i) => {
                                                ...
                                                return (
                                                    <div key={i} className={`flex flex-col bg-gray-900 p-1.5 rounded text-xs text-gray-400 gap-1 ${editingCmIndex === i ? 'border border-blue-500' : ''}`}>
                                                        <div className="flex items-center justify-between">
                                                            <span>
