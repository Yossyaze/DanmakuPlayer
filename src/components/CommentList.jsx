...
const CommentList = forwardRef(({
    ...
    useEffect(() => {
        ...
        console.log(`[CommentList:${debugId}]`, { isAutoScroll, activeCommentId, treeRootsLength: treeRoots.length, hasVirtuosoRef: !!virtuosoRef.current });
        
        if (isAutoScroll && activeCommentId && virtuosoRef.current) {
            const index = treeRoots.findIndex(node => node.id === activeCommentId);
            console.log(`[CommentList:${debugId}] Found index:`, index, 'for activeCommentId:', activeCommentId);
            if (index !== -1) {
                ...
                if (enableTreeView) {
                    ...
                    while (treeEndIndex + 1 < treeRoots.length && treeRoots[treeEndIndex + 1].rootId === rootId) {
                        treeEndIndex++;
                    }
                    const treeSize = treeEndIndex - index + 1;
                    
                    console.log(`[CommentList:${debugId}] Tree size:`, treeSize, 'rootIndex:', index, 'endIndex:', treeEndIndex);
    ...
    const scrollToResNum = useCallback((resNum, sourceFileId) => {
        // Find target in the flattened tree
        // Try same file first
        let index = treeRoots.findIndex(c => c.originalResNum === resNum && c.sourceFileId === sourceFileId);
    ...
    // Handle Anchor Click
    const handleAnchorClick = useCallback((e, resNum, sourceFileId) => {
    ...
    const itemContent = useCallback((index, node) => {
        ...
        return (
            <>
                <RowToRender
                    node={node}
                    isActive={activeCommentId === node.id || (enableTreeView && node.rootId === activeCommentId)}
                ...
                {depthDecreases && (
                    <div 
                        ...
                        style={{ marginLeft: `${Math.max(nextDepth * indentSize, 0)}px` }}
                    />
                )}
            </>
        );
    ...
    }, [RowToRender, activeCommentId, highlightedCommentId, currentLogicalTime, handleAnchorClick, onAnchorMouseEnter, onAnchorMouseLeave, onUrlLoad, showImages, setZoomedImage, timeOffset, handleRowClick, comments.length, onIdClick, showThreadTitle, visibleThreadTitles, onCommentClick, extraRowProps, formatTime, aaMode, aaOverrideMap, enableTreeView, rootTimeMap, treeRoots]);
