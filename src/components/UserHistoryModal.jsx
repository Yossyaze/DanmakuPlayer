                ...
                {contextMenu && (
                    <div className="fixed inset-0 z-100 pointer-events-none">
                        <CommentContextMenu
                            ...
                            onSetLogStart={(c) => { onSetLogStart?.(c); setContextMenu(null); }}
                            onSetCmStart={(t) => { onSetCmStart?.(t); setContextMenu(null); }}
                            onSetCmEnd={(t) => { onSetCmEnd?.(t); setContextMenu(null); }}
                            onAddNgId={(id) => {
                                onAddNgId(id);
                                setContextMenu(null);
                            }}
                            onAddNgComment={(txt) => { onAddNgComment(txt); setContextMenu(null); }}
                            formatTime={formatTime}
                            timeOffset={timeOffset}
                            totalComments={totalComments}
                        />
                            ...
                            {userComments.map((comment, index) => {
                                ...
                                return (
                                    <ItemComponent
                                        ...
                                        totalComments={totalComments}
                                        onClick={(e) => handleRowClick(e, comment)}
                                        className={`px-6 py-3 ${contextMenu?.comment?.id === comment.id ? 'bg-gray-800!' : ''}`}
                                        setZoomedImage={setZoomedImage}
                                        showImages={settings.showImages !== false}
                                        onAnchorClick={onAnchorClick}
                                        onAnchorMouseEnter={onAnchorMouseEnter}
