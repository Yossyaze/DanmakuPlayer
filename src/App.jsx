...
const DesktopApp = () => {
  ...
  useEffect(() => {
      const newSettings = { showThreadTitle, enableTreeView, showImages, imageLayout, aaMode };
  ...
  const { activeDanmaku, danmakuContainerRef, handleAnimationEnd, resetDanmaku, syncLastProcessedTime } = danmaku;
  ...
  const handleVideoUrlSubmit = (e) => handleVideoUrlSubmitFromHook(e, videoUrlInput);
  ...
  useEffect(() => {
    ...
    if (importUrl) {
      logSystem.handleUrlLoad(importUrl).catch(err => console.error("Auto-import failed:", err));
    ...
    const handleMessage = (event) => {
      ...
      if (event.data && event.data.type === 'DANMAKU_IMPORT' && event.data.url) {
        ...
        logSystem.handleUrlLoad(event.data.url).catch(err => alert("Import failed: " + err.message));
      }
    };
          ...
          >
            ...
            {!player.videoSrc ? (
                ...
                <div className="flex flex-col gap-4 w-full max-w-md">
                  ...
                  <label className="flex items-center justify-center gap-3 w-full p-4 bg-gray-800 hover:bg-gray-700 border border-gray-700 hover:border-blue-500 rounded-xl cursor-pointer transition-all group">
                    ...
                    <div className="flex flex-col items-start">
                      <span className="font-bold text-gray-200 group-hover:text-white">動画ファイルを選択</span>
                  ...
                  <form onSubmit={handleVideoUrlSubmit} className="relative group">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      <Link size={16} className="text-gray-500 group-focus-within:text-blue-500 transition-colors" />
                    </div>
                    <input
                      type="text"
                      value={videoUrlInput}
                  ...
                  <div className="flex flex-col gap-2 mt-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        <span className="text-sm text-gray-400 group-hover:text-white font-medium">名前をつけて保存</span>
                      </button>
                      <button
                        onClick={handleImport}
                        className="flex-1 flex items-center justify-center gap-2 p-3 bg-gray-800/50 hover:bg-gray-800 border border-gray-700 hover:border-gray-500 rounded-xl cursor-pointer transition-all group"
                      >
                        <FileInput size={18} className="text-gray-400 group-hover:text-white shrink-0" />
                        <span className="text-sm text-gray-400 group-hover:text-white font-medium whitespace-nowrap">プロジェクトを読み込む</span>
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="w-full h-full relative pointer-events-auto">
                <YouTube
                  videoId={player.videoSrc.includes('v=') ? player.videoSrc.split('v=')[1].split('&')[0] : player.videoSrc} // Extract ID roughly or let library handle if valid? Library wants ID usually.
                  ...
                  onStateChange={(event) => {
                    ...
                    if (event.data === 1) { // Playing
                      ...
                      const isWaiting = cmSystem.cmStateRef.current.isWaiting;
                      console.log(`[App] YouTube Paused. isPlaying:${player.isPlaying} isWaitingRef:${isWaiting}`);
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      {player.videoFileName && (
                        <button
                          onClick={() => {
                            const name = player.videoFileName.replace(/\.[^/.]+$/, ""); // Remove extension
                      ...
                      {/* 3. Thread Title */}
                      {logSystem.loadedFiles.length > 0 && (logSystem.loadedFiles[0].title || logSystem.loadedFiles[0].name) && (
                        <button
                          onClick={() => {
                            let title = logSystem.loadedFiles[0].title || logSystem.loadedFiles[0].name;
                            // Sanitize
                  ...
                  <button
                    onClick={async () => {
                      try {
                        ...
                        const fileName = `${exportFileName || 'project_settings'}.json`;
                        ...
                        if ('showSaveFilePicker' in window) {
                          try {
                            ...
                            setProjectName(exportFileName || 'project_settings');
