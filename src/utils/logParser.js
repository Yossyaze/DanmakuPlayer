...
const parseDat = (buffer, fileId) => {
    ...
    lines.forEach((line, index) => {
        ...
        const body = decodeEntities(parts[3]);
        if (!title && parts[4]) title = decodeEntities(parts[4].trim());
...
const parseHtml = (html, fileId) => {
    ...
    dts.forEach((dt, index) => {
        ...
        const match = dtText.match(/^(\d+)\s*[：:]\s*(.*?)\s*[：:]\s*(\d{4}\/\d{2}\/\d{2}\(.\)\s\d{2}:\d{2}:\d{2}\.\d{2})\s*ID:(.*)$/);
        ...
        parsed.push({
            ...
            userId: userId,
            rawTime: timestamp,
            dateDisplay: dateStr.split(' ')[1] || dateStr,
            text: body,
            color: "#ffffff",
            type: "scroll",
            sourceFileId: fileId
