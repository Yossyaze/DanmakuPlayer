...
export function parseCommentToNodes(comment, options) {
            ...
            if (isImage) {
                ...
                if (imageMode === 'image') {
                    ...
                    if (isValid) {
                        ...
                        nodes.push({ type: 'image', content: url, width: w, height: imageHeight, valid: cachedValidity !== false });
                        ...
                        nodes.push({ type: 'image_error', content: url, text: errorText, width: w });
                        textWidth += w;
                    }
                }
    ...
    if (imageMode === 'placeholder' && imageCount > 0) {
        ...
        nodes.push({ type: 'placeholder', content: '', text, width: w, imageCount });
        textWidth += w;
        hasText = true;
    }
    ...
    if (imageCount > 0 && imageMode === 'image') {
        ...
        nodes.forEach(n => { if (n.type === 'image') n.height = adjustedImageHeight; });
    ...
    if (imageCount > 0 && imageMode === 'image') {
        actualPixelHeight = effectiveLineHeight + imageHeight; // Text row + image row
    }
