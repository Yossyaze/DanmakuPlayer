...
export const useLogSystem = () => {
    ...
    const handleUrlLoad = useCallback(async (urlOrEvent) => {
        let url = urlOrEvent;
        ...
        const resolveDatUrls = (inputUrl) => {
            ...
            // Pattern 1: bbs.eddibb.cc/board/id or bbs.eddibb.cc/test/read.cgi/board/id
            ...
            // Pattern 2: kyodemo.net/sdemo/r/e_e_board/id
            ...
            if (board && id) {
                ...
                if (id.length >= 5) {
                    candidates.push(`https://bbs.eddibb.cc/${board}/kako/${id.slice(0, 4)}/${id.slice(0, 5)}/${id}.dat`);
                }
                return candidates;
            }
            return [inputUrl];
    ...
    const handleToggleFileVisibility = useCallback((fileId) => {
        setLoadedFiles(prev => prev.map(f =>
            f.id === fileId ? { ...f, isVisible: !f.isVisible } : f
        ));
    }, []);
