 ...
 */
export const useLogFilter = (filteredComments = []) => {
    // Search & Filter State
    ...
    }, [searchQuery, searchHistory, searchHistoryIndex, addToHistory]);
    ...
    const displayResults = useMemo(() => {
        if (activeFilter !== 'none') {
            switch (activeFilter) {
                ...
                case 'popular':
                    return filteredComments.filter(c => c.replyCount >= 3)
                ...
                case 'url':
                    ...
                    return filteredComments.filter(c => {
                        ...
                        if (/youtube\.com|youtu\.be|nicovideo\.jp|nico\.ms|twitter\.com|x\.com/i.test(c.text)) return false;
                        return true;
                    });
                case 'video':
                ...
                case 'aa':
                    return filteredComments.filter(c => isProbablyAA(c.text))
