...
export const useLogPopup = (filteredComments = []) => {
    ...
    const handleAnchorClick = useCallback((e, targetResNum, sourceFileId, isNested = false) => {
        if (e) {
            e.stopPropagation();
            e.preventDefault();
        }
        ...
        if (sourceFileId) {
             targetComment = filteredComments.find(c => 
                (c.originalResNum || c.resNum) === targetResNum && c.sourceFileId === sourceFileId
            );
        }
        
        // Fallback if no sourceFileId provided
        if (!targetComment) {
             targetComment = filteredComments.find(c => (c.originalResNum || c.resNum) === targetResNum);
        }
    ...
    const handleReplyCountClick = useCallback((e, comment, isNested = false) => {
        e.stopPropagation();
        ...
        if (!hasAnchors) {
            ...
            const findRoots = (c, visited) => {
                ...
                matches.forEach(m => {
                    ...
                    const parent = filteredComments.find(fc => (fc.originalResNum || fc.resNum) === targetResNum && fc.sourceFileId === c.sourceFileId);
                    if (parent) {
