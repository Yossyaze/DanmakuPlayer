...
export const useDanmakuPlayer = (enableTreeView = false) => {
        ...
        try {
            ...
            if (saved) {
                return { ...{
                    ...
                    opacity: 0.7,
                    area: 100,
                    imageMode: 'none' // none, image, placeholder
                }, ...JSON.parse(saved) };
            }
        } catch (e) {
    ...
    useEffect(() => {
        const extractAndCheckImages = async () => {
            ...
            comments.forEach(c => {
                const matches = c.text.matchAll(/(https?:\/\/[^\s]+\.(jpg|jpeg|png|gif|webp|svg)(\?[^\s]*)?)/gi);
    ...
    const checkCmCollision = useCallback((vidTime, lookAhead = 0) => {
        ...
        for (const range of sortedRanges) {
            ...
            const videoStart = (range.videoStart !== undefined) ? range.videoStart : (range.logStart - accDuration - cmSystem.timeOffset);
    ...
    const togglePlay = () => {
        if (player.isPlaying) {
            ...
            if (cmSystem.cmStateRef.current.isWaiting) {
                ...
                cmSystem.setCurrentCmWaitTime(cmSystem.cmStateRef.current.accumulatedWaitTime);
            } else {
                if (danmakuContainerRef.current) {
    ...
    const processDanmakuFrame = useCallback(() => {
        ...
        if (cmSystem.cmStateRef.current.isWaiting) {
            // [CM MODE]
            const activeRange = cmSystem.cmRanges.find(r => r.id === cmSystem.cmStateRef.current.cmRangeId);

            if (activeRange) {
                if (player.isPlaying) {
                    const elapsed = (performance.now() - cmSystem.cmStateRef.current.waitStartTime) / 1000;
                    // displayTime = CM Start + Accumulated Wait (Ref) + Current Session Elapsed
                    displayTime = activeRange.logStart + cmSystem.cmStateRef.current.accumulatedWaitTime + elapsed;
                    ...
                    if (displayTime >= activeRange.logEnd) {
                        ...
                        cmSystem.setTotalWaitOffset(prev => prev + (activeRange.logEnd - activeRange.logStart));
                    ...
                    if (range) {
                        ...
                        for (const r of [...cmSystem.cmRanges].sort((a, b) => a.logStart - b.logStart)) {
                            if (r.id === justFinishedCmId) {
                                const vs = (r.videoStart !== undefined) ? r.videoStart : (r.logStart - d - cmSystem.timeOffset);
    ...
    }, [videoStartTimeStr, logSystem.comments, logSystem.startTimeStr, cmSystem.setTimeOffset, logSystem, cmSystem]); // Added startTimeStr dependency
    ...
    const activeCommentId = useMemo(() => {
        ...
        for (let i = sourceComments.length - 1; i >= 0; i--) {
            if (sourceComments[i].time <= currentTime) {
                ...
                console.log('[ActiveCommentId]', { currentTime, commentTime: sourceComments[i].time, id: sourceComments[i].id, index: i });
