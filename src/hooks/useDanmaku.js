...
export const useDanmaku = (settings, isPlaying) => {
    ...
    const laneMapRef = useRef({});        // lane -> { startTime, width }
    const rootStateRef = useRef({});      // rootId -> { startTime, lane }
    const lastProcessedTimeRef = useRef(0);
    const skipNextProcessRef = useRef(false); // Skip first process after resume

    const processDanmaku = useCallback((currentDisplayTime, comments, imageValidityMap = null) => {
        ...
        if (skipNextProcessRef.current) {
            ...
            console.log('[ProcessDanmaku] Skipped first frame after resume, synced to:', currentDisplayTime);
            return;
        }

        // Time window for processing
        let searchStart = lastProcessedTimeRef.current;
        if (currentDisplayTime < searchStart || currentDisplayTime - searchStart > 1.0) {
            console.log('[ProcessDanmaku] Time jump detected:', { currentDisplayTime, lastProcessed: searchStart });
            searchStart = currentDisplayTime - 0.1;
        }

        const newComments = comments.filter(c => c.time > searchStart && c.time <= currentDisplayTime);
        
        if (newComments.length > 0) {
            console.log('[ProcessDanmaku] Adding new comments:', { count: newComments.length, searchStart, currentDisplayTime });
            ...
            const childFontScale = 0.9; // Child comments are 90% of parent size
            ...
            const findAvailableLane = (newWidth, rowSpan) => {
                const now = Date.now();
                const newSpeed = (containerWidth + newWidth) / settings.duration;

                for (let lane = 0; lane <= maxLanes - rowSpan; lane++) {
                    let available = true;
                    
                    for (let r = 0; r < rowSpan; r++) {
                        const laneState = laneMapRef.current[lane + r];
                        if (laneState) {
                ...
                // Fallback: random lane (will overlap)
                return Math.floor(Math.random() * Math.max(1, maxLanes - rowSpan + 1));
            };

            // ===== Helper: Register lane occupation =====
            const registerLane = (lane, width, rowSpan) => {
                const now = Date.now();
                for (let r = 0; r < rowSpan; r++) {
                    if (lane + r < maxLanes) {
                        laneMapRef.current[lane + r] = {
                            startTime: now,
                            width: width
                        };
                    }
            ...
            sorted.filter(c => !c.rootId || c.id === c.rootId || c.layoutIndex === 0).forEach(root => {
            ...
            sorted.forEach(c => {
                ...
                const { nodes, totalWidth, rowSpan, actualPixelHeight } = processCommentData(c, !!existingRoot);
                ...
                if (existingRoot) {
                    ...
                    // Child inherits parent's speed to stay aligned
                    ...
                    const parentTop = existingRoot.lane * lineHeight;
                    ...
                    if (childTop + childHeight > displayLimit) {
                        skip = true;
                    } else {
                    ...
                    // Normal comment or root: find available lane using hull size
                    lane = findAvailableLane(hullWidth, hullRowSpan);
                    ...
                    if (c.rootId && c.id === c.rootId) {
                        ...
                        registerLane(lane, hullWidth, hullRowSpan);
                    } else {
                        // Non-tree comment: register with own size
                        registerLane(lane, totalWidth, rowSpan);
                    }
                }
                ...
                if (!skip) {
                    added.push({
                        ...
                        top: c._childTop !== undefined ? c._childTop : lane * lineHeight,
                        dist: dist,
                        width: totalWidth,
            ...
            treeHullMap.forEach((hull, rootId) => {
                if (hull.truncatedCount > 0) {
                    ...
                    if (rootState) {
                        ...
                        const parentTop = rootState.lane * lineHeight;
                        const truncTop = parentTop + lineHeight + displayedHeight;
