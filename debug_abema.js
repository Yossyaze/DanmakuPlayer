...
const ABE_REGEX = new RegExp(
  `(${[...escaped, ...ABE_PATTERNS].join("|")})`,
  "gi"
);

const testString =
  " 18時からabemaで叛逆ほんまにあるやんけ！\nhttps://i.imgur.com/zSCzPJr.png ";
...
if (matches) {
  ...
  // 具体的にどのルールにヒットしたか特定
  matches.forEach((m) => {
    const foundFixed = allFixed.filter(
    ...
    console.log(`  Match "${m}":`);
    if (foundFixed.length > 0)
      console.log(`    Fixed: ${JSON.stringify(foundFixed)}`);
    if (foundPatterns.length > 0)
      console.log(`    Pattern: ${JSON.stringify(foundPatterns)}`);
  });
} else {
